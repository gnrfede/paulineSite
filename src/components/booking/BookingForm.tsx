...
export function BookingForm({ services }: { services: Service[] }) {
  ...
  function toggleService(id: string) {
    setForm((f) => {
      ...
      return {
        ...f,
        serviceIds: already ? f.serviceIds.filter((sid) => sid !== id) : [...f.serviceIds, id],
        date: "",
        timeSlot: "",
      };
    });
  ...
  if (confirmed) {
    const serviceNames = (confirmed.allServices ?? [confirmed.service]).map((s) => s.name).join(", ");
    return (
      <div className="text-center py-12 px-6">
        ...
        <div className="bg-cream-100 rounded-2xl p-5 max-w-sm mx-auto text-left mb-8 space-y-3">
          ...
          <div>
            ...
            <p className="font-sans text-sm font-medium text-gray-700">{formatDate(confirmed.date)} · {confirmed.timeSlot} hs</p>
          </div>
          <div>
            <p className="font-sans text-xs text-gray-400">Código de reserva</p>
            <p className="font-mono text-xs text-teal-600 bg-teal-50 px-2 py-1 rounded mt-1 inline-block">{confirmed.id}</p>
          </div>
        </div>
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          <a href={`https://wa.me/5491134193424?text=Hola Paula! Hice una reserva online: ${serviceNames} para el ${formatDate(confirmed.date)} a las ${confirmed.timeSlot} hs. Código: ${confirmed.id}`}
            target="_blank" rel="noopener noreferrer" className="btn-primary">
            Confirmar por WhatsApp
          </a>
          <button onClick={() => { setConfirmed(null); setForm(INITIAL_FORM); setStep(1); }} className="btn-outline">
            Hacer otra reserva
          </button>
        </div>
      </div>
    ...
    <div>
      ...
      {step === 1 && (
        <div className="space-y-8">
          <div>
            <label className="block font-sans text-xs font-medium tracking-[0.12em] uppercase text-gray-500 mb-1">Elegí uno o más servicios</label>
            ...
            <div className="grid grid-cols-1 gap-2">
              {services.map((srv) => {
                ...
                return (
                  <button key={srv.id} onClick={() => toggleService(srv.id)}
                    className={`text-left p-4 rounded-xl border transition-all duration-150 ${
                      selected ? "border-teal-400 bg-teal-50" : "border-cream-300 bg-white hover:border-teal-200 hover:bg-cream-50"
                    }`}>
                    <div className="flex items-center gap-3">
                      ...
                      <span className={`font-sans text-sm font-medium ${selected ? "text-teal-700" : "text-gray-700"}`}>{srv.name}</span>
                    </div>
                  </button>
                );
              })}
              ...
              <div className="mt-3 flex flex-wrap gap-1.5">
                {selectedServices.map((s) => (
                  <span key={s.id} className="inline-flex items-center gap-1 bg-teal-100 text-teal-700 text-xs font-sans px-2.5 py-1 rounded-full">
                    {s.name}
                    <button onClick={() => toggleService(s.id)} className="ml-0.5 hover:text-teal-900" aria-label={`Quitar ${s.name}`}>×</button>
                  </span>
                ))}
              </div>
            )}
          ...
          {form.serviceIds.length > 0 && (
            <div>
              <label className="block font-sans text-xs font-medium tracking-[0.12em] uppercase text-gray-500 mb-3">Elegí una fecha</label>
              <BookingCalendar selectedDate={form.date} onSelectDate={(d) => setField("date", d)}
          ...
          {form.date && (
            <div>
              <label className="block font-sans text-xs font-medium tracking-[0.12em] uppercase text-gray-500 mb-3">Elegí un horario</label>
              <TimeSlots slots={slots} selected={form.timeSlot} onSelect={(t) => setField("timeSlot", t)} loading={slotsLoading} />
              {errors.timeSlot && <p className="font-sans text-xs text-red-500 mt-2">{errors.timeSlot}</p>}
            </div>
          )}

          <button onClick={nextStep} disabled={form.serviceIds.length === 0 || !form.date || !form.timeSlot}
      ...
      {step === 2 && (
        <div className="space-y-5">
          <div className="bg-teal-50 rounded-xl p-4 border border-teal-100">
            <p className="font-sans text-xs text-teal-600 font-medium mb-1">{selectedServices.map((s) => s.name).join(" + ")}</p>
            <p className="font-sans text-sm text-gray-600">{formatDate(form.date)} · {form.timeSlot} hs</p>
          </div>

          <div>
            <label className="block font-sans text-xs font-medium text-gray-500 mb-1.5">Nombre completo *</label>
            <input type="text" value={form.name} onChange={(e) => setField("name", e.target.value)}
              placeholder="Tu nombre completo" className={`input-field ${errors.name ? "border-red-300" : ""}`} />
          ...
          <div>
            ...
            <input type="email" value={form.email} onChange={(e) => setField("email", e.target.value)}
              placeholder="tu@email.com" className={`input-field ${errors.email ? "border-red-300" : ""}`} />
          ...
          <div>
            ...
            <input type="tel" value={form.phone} onChange={(e) => setField("phone", e.target.value)}
              placeholder="+54 11 1234 5678" className={`input-field ${errors.phone ? "border-red-300" : ""}`} />
          ...
          <div>
            ...
            <textarea value={form.notes} onChange={(e) => setField("notes", e.target.value)}
              placeholder="Contáme sobre tu piel, alergias o cualquier consulta. Si necesitás algún producto, podés solicitarlo con anticipación y lo dejamos preparado para tu turno."
              rows={3} className="input-field resize-none" />
          </div>

          {/* Newsletter consent — unchecked by default (GDPR compliant) */}
          <label className="flex items-start gap-3 cursor-pointer group">
            <div className="mt-0.5 shrink-0">
              ...
              <div className={`w-4 h-4 rounded border-2 flex items-center justify-center transition-colors ${
                form.newsletterConsent ? "bg-teal-400 border-teal-400" : "border-gray-300 group-hover:border-teal-300"
              }`}>
            ...
            <span className="font-sans text-xs text-gray-500 leading-relaxed">
              Quiero recibir novedades, consejos de cuidado de piel y promociones de Pauline Studio.
              <span className="text-gray-400"> Podés cancelar tu suscripción en cualquier momento.</span>
            </span>
          </label>
